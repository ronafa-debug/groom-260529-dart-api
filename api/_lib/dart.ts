...
import staticCorpCodes from '../_data/corp-codes.json' with { type: 'json' };
import { fetchDartCorpCodeList, getDartApiKey } from './corp-codes-fetch.js';

const DART_BASE = 'https://opendart.fss.or.kr/api';

function getApiKey(): string {
  return getDartApiKey();
}
...
export async function getCorpCodeList(): Promise<CorpCode[]> {
  const bundled = staticCorpCodes as CorpCode[];
  if (bundled.length > 0) return bundled;

  return cacheGetOrSet('corp:list', 86400, fetchDartCorpCodeList);
}
